...
const servidor = http.createServer((request, response) => {
  ...
  if (request.method === "GET" && request.url === "/") {
    ...
    response.write("Bienvenida comunidad. Este es el servidor de participación de María.")
    response.end()
    return
  }
  ...
  if (request.method === "GET" && request.url === "/propuestas") {
    response.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" })
    response.write("Lista de propuestas: convivencia comunitaria, información transparente.")
    response.end()
    return
  }
